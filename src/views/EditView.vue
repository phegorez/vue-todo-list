...
<template>
    ...
    <div class="w-1/2 mx-auto fixed inset-0 flex flex-col justify-center" v-else>
        ...
        <div class="flex items-center justify-between">
            <RouterLink to="/">
                <button class="btn btn-square btn-outline"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"
                        height="1.5em">
                        <path
        ...
        <div class="form-control w-full">
            ...
            <input type="text" placeholder="Type name todo" v-model="useTodo.selectedTodo.name"
        ...
        <div class="form-control w-full">
            ...
            <select v-model="useTodo.selectedTodo.status" class="select select-bordered w-full"
                @change="changeStatus(useTodo.selectedTodo, useTodo.selectedTodo.id)">
                <option v-for="status in useTodo.statuses">{{ status }}</option>
            </select>
        </div>
        <button @click="editTodo(useTodo.selectedTodo, useTodo.selectedTodo.id)"
            class="btn btn-primary w-full mt-4">EDIT</button>
    </div>
</template>
